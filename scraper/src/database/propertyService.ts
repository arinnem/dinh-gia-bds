...
export class PropertyService {
  ...
  async insertProperty(scrapedProperty: ScrapedProperty): Promise<number | null> {
    ...
    try {
      ...
      const propertyParams = [
        ...
        DataProcessor.cleanText(scrapedProperty.contact.name || ''),
  ...
  private async getOrCreatePropertyType(client: any, typeName: string): Promise<number> {
    ...
    const newType = await client.query(
      'INSERT INTO property_types (name, description, created_at, updated_at) VALUES ($1, $2, NOW(), NOW()) RETURNING id',
      [cleanTypeName, `Auto-created from scraping: ${cleanTypeName}`]
    );
  ...
  private async getOrCreateLegalStatus(client: any, statusName: string): Promise<number> {
    ...
    const newStatus = await client.query(
      'INSERT INTO legal_statuses (name, description, created_at, updated_at) VALUES ($1, $2, NOW(), NOW()) RETURNING id',
      [cleanStatusName, `Auto-created from scraping: ${cleanStatusName}`]
    );
  ...
  private async getOrCreateDirection(client: any, directionName: string): Promise<number> {
    ...
    const newDirection = await client.query(
      'INSERT INTO directions (name, description, created_at, updated_at) VALUES ($1, $2, NOW(), NOW()) RETURNING id',
      [cleanDirectionName, `Auto-created from scraping: ${cleanDirectionName}`]
    );
  ...
  private async getOrCreateProject(client: any, projectName: string, districtId: number | null): Promise<number> {
    ...
    const newProject = await client.query(
      'INSERT INTO projects (name, description, district_id, created_at, updated_at) VALUES ($1, $2, $3, NOW(), NOW()) RETURNING id',
      [cleanProjectName, `Auto-created from scraping: ${cleanProjectName}`, districtId]
    );
   ...
   * Insert property images
   */
  private async insertPropertyImages(client: any, propertyId: number, images: string[]): Promise<void> {
    for (let i = 0; i < images.length; i++) {
      await client.query(
        'INSERT INTO property_images (property_id, image_url, alt_text, display_order, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW())',
        [propertyId, images[i], `Property image ${i + 1}`, i + 1]
      );
    }
  }
   ...
   * Insert price history
   */
  private async insertPriceHistory(client: any, propertyId: number, price: number): Promise<void> {
    await client.query(
      'INSERT INTO price_history (property_id, price, recorded_date, source, created_at, updated_at) VALUES ($1, $2, NOW(), $3, NOW(), NOW())',
      [propertyId, price, 'scraper']
    );
  }
